...
GEM5 enable-difftest 在 `eda-00` 上使用独立结果路径 `build/validation/l3-gem5-enable-difftest-eda00/supported-cross-77/results.json`，汇总目标名为 `gem5-isa-difftest-eda00`。在默认远程 `linux aarch64` 上使用本机源码构建的 Spike ref so 时，当前修复后结果路径为 `build/validation/l3-gem5-spikeproxy-fix-linux-aarch64/supported-cross-77/results.json`，汇总目标名为 `gem5-isa-difftest-linux-aarch64-spike`。这些结果不会覆盖旧的非 difftest `gem5-isa-first-layer`，用于明确区分“GEM5 raw-cpt L3可运行”和“GEM5+ref so difftest可运行”这两种证明边界。
