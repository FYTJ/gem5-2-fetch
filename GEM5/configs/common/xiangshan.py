...
def _finish_xiangshan_system(args, test_sys, TestCPUClass, ruby):
    ...
    for i in range(np):
        ...
        test_sys.cpu[i].branchPred = DecoupledBPUWithBTB(
            ...
            enableTwoAheadTwoTakenShadow=bool(getattr(args, 'enable_bpu_two_ahead_shadow', False)),
            enableTwoAheadTwoTakenFunctional=bool(getattr(args, 'enable_bpu_two_ahead_functional', False)),
            enableTwoFetch=bool(getattr(args, 'enable_bpu_two_fetch', False)),
            enableTwoFetchTrace=not bool(getattr(args, 'disable_bpu_two_fetch_trace', False)),
            enableTwoFetchEnhancedGate=not bool(getattr(args, 'disable_bpu_two_fetch_enhanced_gate', False)),
            enableTwoFetchSyntheticFallback=bool(getattr(args, 'enable_bpu_two_fetch_synthetic_fallback', False)),
            twoFetchOracleMode=str(getattr(args, 'bpu_two_fetch_oracle_mode', 'disabled')),
            twoFetchGateMinFreeFTQ=int(getattr(args, 'bpu_two_fetch_gate_min_free_ftq', 3)),
            twoFetchUsefulnessTableSize=int(getattr(args, 'bpu_two_fetch_usefulness_table_size', 2048)),
            twoFetchUsefulnessThreshold=int(getattr(args, 'bpu_two_fetch_usefulness_threshold', -1)),
            twoFetchUsefulnessMax=int(getattr(args, 'bpu_two_fetch_usefulness_max', 3)),
            twoFetchUsefulnessExploreInterval=int(getattr(args, 'bpu_two_fetch_usefulness_explore_interval', 16)),
            twoFetchUsefulnessExploreMinFreeFTQ=int(getattr(args, 'bpu_two_fetch_usefulness_explore_min_free_ftq', 4)),
        )
