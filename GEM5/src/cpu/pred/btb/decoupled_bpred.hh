...
class DecoupledBPUWithBTB : public BPredUnit
{
  ...
  private:
    ...
    bool enableTwoAheadTwoTakenShadow{false};
    bool enableTwoAheadTwoTakenFunctional{false};
    bool enableTwoFetch{false};
    bool enableTwoFetchTrace{true};
    bool enableTwoFetchEnhancedGate{true};
    bool enableTwoFetchSyntheticFallback{false};
    std::string twoFetchOracleMode{"disabled"};
    unsigned twoFetchGateMinFreeFTQ{3};
    int twoFetchUsefulnessThreshold{-1};
    int twoFetchUsefulnessMax{3};
    unsigned twoFetchUsefulnessExploreInterval{16};
    unsigned twoFetchUsefulnessExploreMinFreeFTQ{4};
    ...
    OutputStream *twoAheadTraceStream{nullptr};
    uint64_t twoAheadEpoch[MaxThreads]{};
    uint64_t twoAheadFlushSeq{0};

    void initDB();
    Addr shadowFallthrough(Addr start) const;
    std::string shadowMetadataId(ThreadID tid, FetchTargetId fetch_id,
                                 uint64_t epoch) const;
    std::vector<BTBEntry> collectShadowTakenCandidates(FullBTBPrediction &pred);
    void writeTwoAheadPredictTrace(ThreadID tid, FetchTargetId fetch_id,
                                   FetchTarget &entry,
                                   FullBTBPrediction &pred,
                                   const FetchTarget *window1_entry = nullptr,
                                   FetchTargetId window1_fetch_id = 0);
    void writeTwoAheadLifecycleTrace(const char *event, ThreadID tid,
                                     FetchTargetId fetch_id,
                                     const FetchTarget &target);
    void writeTwoFetchSupplyTrace(const char *event, ThreadID tid,
                                  FetchTargetId fetch_id,
                                  uint64_t stream_id, uint64_t epoch,
                                  Addr cacheline_base, unsigned line_index,
                                  const char *destination,
                                  const char *reason,
                                  bool stale, unsigned bytes = 0);
    uint64_t writeTwoAheadRedirectTrace(ThreadID tid, FetchTargetId target_id,
                                        Addr from_pc, Addr redirect_pc,
                                        const char *reason);
    void writeTwoAheadFlushTrace(ThreadID tid, FetchTargetId begin_id,
                                 FetchTargetId end_id, uint64_t flush_id,
                                 const char *reason);
    bool isTwoFetchWindow1(const FetchTarget &target) const;
    void accountTwoFetchWindow1HistorySnapshot(const FetchTarget &target);
    void accountTwoFetchWindow1HistoryRestore(const FetchTarget &target);
    ...
    std::vector<int8_t> twoFetchUsefulnessTable;
    std::vector<uint64_t> twoFetchUsefulnessExploreCounters;
    ...
    FullBTBPrediction selectFinalPrediction(
        ThreadID tid,
        std::vector<FullBTBPrediction> &preds_of_each_stage,
        bool update_predictor_side_effects,
        bool update_stats);
    FullBTBPrediction predictTwoFetchWindow1(ThreadID tid, Addr start_pc);
    FetchTarget createFetchTargetFromPrediction(ThreadID tid,
                                                Addr start_pc,
                                                FullBTBPrediction &pred,
                                                uint64_t shadow_epoch);
    FetchTarget createFetchTargetEntry(ThreadID tid);
    FetchTarget createTwoFetchWindow1Entry(ThreadID tid,
                                           const FetchTarget &parent,
                                           FetchTargetId parent_id,
                                           Addr start_pc,
                                           FullBTBPrediction &pred);
    unsigned twoFetchUsefulnessIndex(const FetchTarget &parent,
                                     Addr window1_pc) const;
    bool shouldAcceptTwoFetchWindow1(const FetchTarget &parent,
                                     Addr window1_pc,
    ...
    bool shouldExploreTwoFetchUsefulness(ThreadID tid,
                                         unsigned usefulness_idx);
    void updateTwoFetchUsefulness(
        const FetchTarget &target, int delta,
        uint32_t reason = TWO_FETCH_USEFULNESS_UPDATE_GENERIC);
    void updateTwoFetchUsefulnessIndex(
        unsigned idx, int delta,
        uint32_t reason = TWO_FETCH_USEFULNESS_UPDATE_GENERIC);
    ...
    void fillAheadPipeline(FetchTarget &entry,
                           FetchTargetId fetch_id,
                           const FetchTarget *pending_target = nullptr,
                           FetchTargetId pending_target_id = 0);
    ...
    struct DBPBTBStats : public statistics::Group {
        ...
        statistics::Scalar twoAheadFunctionalCarried;
        statistics::Scalar twoAheadFunctionalSecondWindowCarried;
        statistics::Scalar twoAheadFunctionalTwoTakenCarried;
        statistics::Scalar twoFetchBatchCreated;
        statistics::Scalar twoFetchWindow1Materialized;
        statistics::Scalar twoFetchWindow1Enqueued;
        statistics::Scalar twoFetchWindow1Consumed;
        statistics::Scalar twoFetchWindow1ConsumedInsts;
        statistics::Scalar twoFetchWindow1Squashed;
        statistics::Scalar twoFetchFallbackFtqCapacity;
        statistics::Scalar twoFetchFallbackInvalidPC;
        statistics::Scalar twoFetchWindow1Candidate;
        statistics::Scalar twoFetchWindow1PreLookupRejected;
        statistics::Scalar twoFetchWindow1PostLookupRejected;
        statistics::Scalar twoFetchWindow1PredLookup;
        statistics::Scalar twoFetchWindow1PredHit;
        statistics::Scalar twoFetchWindow1PredTaken;
        statistics::Scalar twoFetchWindow1PredReturn;
        statistics::Scalar twoFetchWindow1PredIndirect;
        statistics::Scalar twoFetchWindow1Accepted;
        statistics::Scalar twoFetchWindow1SyntheticFallback;
        statistics::Scalar twoFetchWindow1RejectedFtqCapacity;
        statistics::Scalar twoFetchWindow1RejectedInvalidPC;
        statistics::Scalar twoFetchWindow1RejectedGate;
        statistics::Scalar twoFetchWindow1RejectedSyntheticFallback;
        statistics::Scalar twoFetchWindow1RejectedUsefulness;
        statistics::Scalar twoFetchWindow1RejectedParentRisk;
        statistics::Scalar twoFetchWindow1RejectedFrontendPressure;
        statistics::Scalar twoFetchWindow1RejectedPredMiss;
        statistics::Scalar twoFetchWindow1RejectedSupplyRisk;
        statistics::Scalar twoFetchWindow1AcceptedByExploration;
        statistics::Scalar twoFetchUsefulnessExploreCandidate;
        statistics::Scalar twoFetchUsefulnessExploreAllowed;
        statistics::Scalar twoFetchUsefulnessExploreBudgetBlocked;
        statistics::Scalar twoFetchUsefulnessExploreResourceBlocked;
        statistics::Scalar twoFetchWindow1SquashedByWindow0;
        statistics::Scalar twoFetchWindow1SquashedBySelf;
        statistics::Scalar twoFetchWindow1SquashedBeforeFetch;
        statistics::Scalar twoFetchWindow1ParentInvalidated;
        statistics::Scalar twoFetchWindow1ParentInvalidatedTrainBlocked;
        statistics::Scalar twoFetchWindow1UsefulInsts;
        statistics::Scalar twoFetchWindow1CommittedInsts;
        statistics::Scalar twoFetchWindow1StreamCreated;
        statistics::Scalar twoFetchWindow1StreamAccepted;
        statistics::Scalar twoFetchWindow1StreamConsumed;
        statistics::Scalar twoFetchWindow1StreamInvalidated;
        statistics::Scalar twoFetchWindow1StreamParentInvalidated;
        statistics::Scalar twoFetchWindow1StreamSelfInvalidated;
        statistics::Scalar twoFetchHistorySnapshot;
        statistics::Scalar twoFetchHistoryRestore;
        statistics::Scalar twoFetchRasSnapshot;
        statistics::Scalar twoFetchRasRestore;
        statistics::Scalar twoFetchWindow1WrongPathFetchInsts;
        statistics::Scalar twoFetchWindow1IcachePrefetchReq;
        statistics::Scalar twoFetchWindow1IcachePrefetchEligible;
        statistics::Scalar twoFetchWindow1IcachePrefetchQueued;
        statistics::Scalar twoFetchWindow1SupplyQueueScanned;
        statistics::Scalar twoFetchWindow1SupplyQueueSelected;
        statistics::Scalar twoFetchWindow1SupplyQueueEmpty;
        statistics::Scalar twoFetchWindow1SupplyQueueBlockedOutstanding;
        statistics::Scalar twoFetchWindow1SupplyQueueBlockedDemandBusy;
        statistics::Scalar twoFetchWindow1SupplyQueueBlockedCacheBlocked;
        statistics::Scalar twoFetchWindow1SupplyQueueBlockedRetry;
        statistics::Scalar twoFetchWindow1SupplyQueueBlockedTooClose;
        statistics::Scalar twoFetchWindow1SupplyQueueDistanceSamples;
        statistics::Scalar twoFetchWindow1SupplyQueueDistanceTotal;
        statistics::Scalar twoFetchWindow1IcachePrefetchIssueLine0;
        statistics::Scalar twoFetchWindow1IcachePrefetchIssueLine1;
        statistics::Scalar twoFetchWindow1IcachePrefetchBlockedRecoveryRisk;
        statistics::Scalar twoFetchWindow1IcachePrefetchBlockedState;
        statistics::Scalar twoFetchWindow1IcachePrefetchBlockedDemandBusy;
        statistics::Scalar twoFetchWindow1IcachePrefetchBlockedNoBytes;
        statistics::Scalar twoFetchWindow1IcachePrefetchTooLate;
        statistics::Scalar twoFetchWindow1IcachePrefetchLateResponse;
        statistics::Scalar twoFetchWindow1IcachePrefetchHit;
        statistics::Scalar twoFetchWindow1IcachePrefetchMiss;
        statistics::Scalar twoFetchWindow1IcachePrefetchDropped;
        statistics::Scalar twoFetchWindow1IcachePrefetchUseful;
        statistics::Scalar twoFetchWindow1IcachePrefetchBackpressure;
        statistics::Scalar twoFetchWindow1IcachePrefetchReady;
        statistics::Scalar twoFetchWindow1IcachePrefetchRetry;
        statistics::Scalar twoFetchWindow1IcachePrefetchStaleResponse;
        statistics::Scalar twoFetchWindow1IcachePrefetchDemandPreempt;
        statistics::Scalar twoFetchWindow1IcachePrefetchDuplicate;
        statistics::Scalar twoFetchWindow1IcachePrefetchPromotionFull;
        statistics::Scalar twoFetchWindow1IcachePrefetchPromotionPartial;
        statistics::Scalar twoFetchWindow1IcachePrefetchPromotionMiss;
        statistics::Scalar twoFetchWindow1IcachePrefetchPromotionStale;
        statistics::Scalar twoFetchWindow1IcachePrefetchBufferEvicted;
        statistics::Scalar twoFetchWindow1IcachePrefetchReadyButNotConsumed;
        statistics::Scalar twoFetchWindow1IcachePrefetchReadyButCancelled;
        statistics::Scalar twoFetchWindow1IcachePrefetchReadyBeforeDemand;
        statistics::Scalar twoFetchWindow1IcachePrefetchConsumedWithoutReady;
        statistics::Scalar twoFetchWindow1IcachePrefetchMaterializeToReqSamples;
        statistics::Scalar twoFetchWindow1IcachePrefetchMaterializeToReqTicks;
        statistics::Scalar twoFetchWindow1IcachePrefetchReqToReadySamples;
        statistics::Scalar twoFetchWindow1IcachePrefetchReqToReadyTicks;
        statistics::Scalar twoFetchWindow1IcachePrefetchReadyToHeadSamples;
        statistics::Scalar twoFetchWindow1IcachePrefetchReadyToHeadTicks;
        statistics::Scalar twoFetchWindow1IcachePrefetchHeadToPromotionSamples;
        statistics::Scalar twoFetchWindow1IcachePrefetchHeadToPromotionTicks;
        statistics::Scalar twoFetchOracleFunctionalFillAttempt;
        statistics::Scalar twoFetchOracleFunctionalFillSuccess;
        statistics::Scalar twoFetchOracleFunctionalFillTranslationFault;
        statistics::Scalar twoFetchOracleFunctionalFillInvalidPaddr;
        statistics::Scalar twoFetchOracleFunctionalFillBytes;
        statistics::Scalar twoFetchFrontendOracleDemandFillAttempt;
        statistics::Scalar twoFetchFrontendOracleDemandFillSuccess;
        statistics::Scalar twoFetchFrontendOracleDemandFillTranslationFault;
        statistics::Scalar twoFetchFrontendOracleDemandFillInvalidPaddr;
        statistics::Scalar twoFetchFrontendOracleDemandFillBytes;
        statistics::Scalar twoFetchUsefulnessUpdatePositive;
        statistics::Scalar twoFetchUsefulnessUpdateNegative;
        statistics::Scalar twoFetchUsefulnessUpdateClamped;
        statistics::Scalar twoFetchUsefulnessUpdateRewardCommit;
        statistics::Scalar twoFetchUsefulnessUpdateRewardPrefetchUseful;
        statistics::Scalar twoFetchUsefulnessUpdateCostParentInvalidated;
        statistics::Scalar twoFetchUsefulnessUpdateCostSelfSquash;
        statistics::Scalar twoFetchUsefulnessUpdateCostWrongPath;
        statistics::Scalar twoFetchUsefulnessUpdateCostPrefetchBackpressure;
        statistics::Scalar twoFetchUsefulnessUpdateCostPromotionMiss;
        statistics::Scalar twoFetchUsefulnessUpdateCostSupplyRiskReject;
        statistics::Scalar twoFetchUsefulnessUpdateExplorationRecovery;
  ...
  public:
    ...
    bool ftqHasNextFetchingTarget(ThreadID tid) const
    {
        return ftq.hasTarget(ftq.fetchId(tid) + 1, tid);
    }
    FetchTargetId ftqNextFetchingTargetId(ThreadID tid) const
    {
        assert(ftqHasNextFetchingTarget(tid));
        return ftq.fetchId(tid) + 1;
    }
    const FetchTarget &ftqNextFetchingTarget(ThreadID tid)
    {
        assert(ftqHasNextFetchingTarget(tid));
    ...
    FetchTargetId ftqBackId(ThreadID tid) const
    {
        ...
        return ftq.backId(tid);
    }
    bool ftqHasTargetId(ThreadID tid, FetchTargetId fetch_id) const
    {
        return ftq.hasTarget(fetch_id, tid);
    }
    const FetchTarget &ftqTarget(ThreadID tid, FetchTargetId fetch_id)
    {
        assert(ftqHasTargetId(tid, fetch_id));
        return ftq.get(fetch_id, tid);
    }
    bool isLiveTwoFetchStream(ThreadID tid, FetchTargetId fetch_id,
                              uint64_t stream_id, uint64_t epoch);
    bool twoFetchOracleIdealReady() const
    {
        return enableTwoFetch && twoFetchOracleMode == "ideal-ready";
    }
    bool twoFetchOracleIdealPromotion() const
    {
        return enableTwoFetch && twoFetchOracleMode == "ideal-promotion";
    }
    bool twoFetchOracleIdealFrontend() const
    {
        return enableTwoFetch && twoFetchOracleMode == "ideal-frontend";
    }
    bool twoFetchOracleEnabled() const
    {
        return twoFetchOracleIdealReady() || twoFetchOracleIdealPromotion() ||
               twoFetchOracleIdealFrontend();
    }
    void noteTwoFetchFrontendOracleDemandFill(
        ThreadID tid, FetchTargetId fetch_id, Addr start_pc,
        const char *result, unsigned bytes = 0);
    void noteTwoFetchPrefetchEligible(ThreadID tid, FetchTargetId fetch_id,
                                      uint64_t stream_id, uint64_t epoch,
                                      Addr cacheline_base);
    void noteTwoFetchPrefetchQueued(ThreadID tid, FetchTargetId fetch_id,
                                    uint64_t stream_id, uint64_t epoch,
                                    Addr cacheline_base);
    void noteTwoFetchSupplyQueueScan(ThreadID tid, FetchTargetId fetch_id,
                                     uint64_t stream_id, uint64_t epoch,
                                     Addr cacheline_base,
                                     unsigned ftq_distance);
    void noteTwoFetchSupplyQueueSelected(ThreadID tid,
                                         FetchTargetId fetch_id,
                                         uint64_t stream_id,
                                         uint64_t epoch,
                                         Addr cacheline_base,
                                         unsigned ftq_distance,
                                         unsigned line_index);
    void noteTwoFetchSupplyQueueEmpty(ThreadID tid);
    void noteTwoFetchSupplyQueueBlocked(ThreadID tid,
                                        FetchTargetId fetch_id,
                                        uint64_t stream_id,
                                        uint64_t epoch,
                                        Addr cacheline_base,
                                        const char *reason);
    void noteTwoFetchPrefetchBlocked(ThreadID tid, FetchTargetId fetch_id,
                                     uint64_t stream_id, uint64_t epoch,
                                     Addr cacheline_base,
                                     const char *reason);
    void noteTwoFetchPrefetchRequest(ThreadID tid, FetchTargetId fetch_id,
                                     uint64_t stream_id, uint64_t epoch,
                                     Addr cacheline_base, unsigned line_index);
    void noteTwoFetchPrefetchMaterializeToRequest(Tick ticks);
    void noteTwoFetchPrefetchResponse(ThreadID tid, FetchTargetId fetch_id,
                                      uint64_t stream_id, uint64_t epoch,
                                      Addr cacheline_base, unsigned line_index,
                                      const char *destination, bool stale);
    void noteTwoFetchPrefetchLateResponse(ThreadID tid,
                                          FetchTargetId fetch_id,
                                          uint64_t stream_id,
                                          uint64_t epoch,
                                          Addr cacheline_base,
                                          unsigned line_index);
    void noteTwoFetchPrefetchRequestToReady(Tick ticks);
    void noteTwoFetchPrefetchDrop(ThreadID tid, FetchTargetId fetch_id,
                                  uint64_t stream_id, uint64_t epoch,
                                  Addr cacheline_base, unsigned line_index,
                                  const char *reason, bool stale = false);
    void noteTwoFetchPrefetchCancel(ThreadID tid, FetchTargetId fetch_id,
                                    uint64_t stream_id, uint64_t epoch,
                                    Addr cacheline_base, unsigned line_index,
                                    const char *reason);
    void noteTwoFetchPrefetchBackpressure(ThreadID tid, FetchTargetId fetch_id,
                                          uint64_t stream_id, uint64_t epoch,
                                          Addr cacheline_base,
                                          unsigned line_index,
                                          const char *reason);
    void noteTwoFetchPrefetchDemandPreempt(ThreadID tid, FetchTargetId fetch_id,
                                           uint64_t stream_id, uint64_t epoch,
                                           Addr cacheline_base,
                                           unsigned line_index,
                                           const char *reason);
    void noteTwoFetchPrefetchDuplicate(ThreadID tid, FetchTargetId fetch_id,
                                       uint64_t stream_id, uint64_t epoch,
                                       Addr cacheline_base,
                                       unsigned line_index);
    void noteTwoFetchPrefetchPromotion(ThreadID tid, FetchTargetId fetch_id,
                                       uint64_t stream_id, uint64_t epoch,
                                       Addr cacheline_base,
                                       const char *reason,
                                       unsigned bytes, bool full);
    void noteTwoFetchPrefetchReadyToHead(Tick ticks);
    void noteTwoFetchPrefetchHeadToPromotion(Tick ticks);
    void noteTwoFetchPrefetchReadyBeforeDemand(ThreadID tid,
                                               FetchTargetId fetch_id,
                                               uint64_t stream_id,
                                               uint64_t epoch,
                                               Addr cacheline_base,
                                               unsigned ready_bytes);
    void noteTwoFetchPrefetchUseful(ThreadID tid, FetchTargetId fetch_id,
                                    FetchTarget &target);
    void noteTwoFetchPrefetchPromotionMiss(ThreadID tid,
                                           FetchTargetId fetch_id,
                                           uint64_t stream_id, uint64_t epoch,
                                           Addr cacheline_base,
                                           const char *reason, bool stale);
    void noteTwoFetchPrefetchEvict(ThreadID tid, FetchTargetId fetch_id,
                                   uint64_t stream_id, uint64_t epoch,
                                   Addr cacheline_base, bool used,
                                   bool any_ready, bool cancelled);
    void noteTwoFetchPrefetchReadyDiscard(ThreadID tid,
                                          FetchTargetId fetch_id,
                                          uint64_t stream_id,
                                          uint64_t epoch,
                                          Addr cacheline_base,
                                          const char *reason);
    void noteTwoFetchOracleFunctionalFill(ThreadID tid,
                                          FetchTargetId fetch_id,
                                          uint64_t stream_id,
                                          uint64_t epoch,
                                          Addr cacheline_base,
