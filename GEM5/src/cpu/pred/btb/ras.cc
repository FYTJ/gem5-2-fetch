...
std::shared_ptr<void>
BTBRAS::getSyntheticNoHitMeta(ThreadID tid)
{
    if (tid >= threadStates.size()) {
        return nullptr;
    }
    ...
    auto &state = threadStates[tid];
    auto meta = std::make_shared<RASMeta>();
    meta->ssp = state.ssp;
    meta->sctr = state.sctr;
    meta->TOSR = state.TOSR;
    meta->TOSW = state.TOSW;
    meta->willPush = false;
    if (inflightInRange(state, state.TOSR)) {
        meta->target = state.inflightStack[state.TOSR].data.retAddr;
    } else {
        meta->target = state.stack[state.ssp].data.retAddr;
    }
    return meta;
}
