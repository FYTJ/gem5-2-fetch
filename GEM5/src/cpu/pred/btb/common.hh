...
enum TwoFetchPredictionKind : uint8_t
{
    TWO_FETCH_PRED_NONE = 0,
    TWO_FETCH_PRED_SYNTHETIC_FALLBACK = 1,
    TWO_FETCH_PRED_PREDICTOR_BACKED_CANDIDATE = 2,
    TWO_FETCH_PRED_PREDICTOR_BACKED_ACCEPTED = 3,
};

enum TwoFetchPrefetchState : uint8_t
{
    TWO_FETCH_PREFETCH_NOT_ISSUED = 0,
    TWO_FETCH_PREFETCH_ISSUED = 1,
    TWO_FETCH_PREFETCH_READY = 2,
    TWO_FETCH_PREFETCH_USED = 3,
    TWO_FETCH_PREFETCH_DROPPED = 4,
    TWO_FETCH_PREFETCH_CANCELLED = 5,
    TWO_FETCH_PREFETCH_STALE_RESPONSE = 6,
};

enum TwoFetchRejectReason : uint32_t
{
    TWO_FETCH_REJECT_NONE = 0,
    TWO_FETCH_REJECT_FTQ_CAPACITY = 1 << 0,
    TWO_FETCH_REJECT_INVALID_PC = 1 << 1,
    TWO_FETCH_REJECT_SYNTHETIC_DISABLED = 1 << 2,
    TWO_FETCH_REJECT_USEFULNESS = 1 << 3,
    TWO_FETCH_REJECT_PARENT_RISK = 1 << 4,
    TWO_FETCH_REJECT_FRONTEND_PRESSURE = 1 << 5,
    TWO_FETCH_REJECT_PREDICTOR_MISS = 1 << 6,
    TWO_FETCH_REJECT_SUPPLY_RISK = 1 << 7,
};

enum TwoFetchSupplyRisk : uint32_t
{
    TWO_FETCH_SUPPLY_RISK_NONE = 0,
    TWO_FETCH_SUPPLY_RISK_PARENT_TAKEN = 1 << 0,
    TWO_FETCH_SUPPLY_RISK_PARENT_OVERRIDE = 1 << 1,
    TWO_FETCH_SUPPLY_RISK_WINDOW1_TAKEN = 1 << 2,
    TWO_FETCH_SUPPLY_RISK_WINDOW1_RETURN = 1 << 3,
    TWO_FETCH_SUPPLY_RISK_WINDOW1_INDIRECT = 1 << 4,
    TWO_FETCH_SUPPLY_RISK_CROSS_CACHELINE = 1 << 5,
    TWO_FETCH_SUPPLY_RISK_CROSS_PAGE = 1 << 6,
    TWO_FETCH_SUPPLY_RISK_LONG_WINDOW = 1 << 7,
    TWO_FETCH_SUPPLY_RISK_WINDOW1_CALL = 1 << 8,
};

enum TwoFetchUsefulnessUpdateReason : uint32_t
{
    TWO_FETCH_USEFULNESS_UPDATE_GENERIC = 0,
    TWO_FETCH_USEFULNESS_UPDATE_COMMIT = 1,
    TWO_FETCH_USEFULNESS_UPDATE_PREFETCH_USEFUL = 2,
    TWO_FETCH_USEFULNESS_UPDATE_PARENT_INVALIDATED = 3,
    TWO_FETCH_USEFULNESS_UPDATE_SELF_SQUASH = 4,
    TWO_FETCH_USEFULNESS_UPDATE_WRONG_PATH = 5,
    TWO_FETCH_USEFULNESS_UPDATE_PREFETCH_BACKPRESSURE = 6,
    TWO_FETCH_USEFULNESS_UPDATE_PROMOTION_MISS = 7,
    TWO_FETCH_USEFULNESS_UPDATE_SUPPLY_RISK_REJECT = 8,
    TWO_FETCH_USEFULNESS_UPDATE_EXPLORATION_RECOVERY = 9,
};
...
struct FetchTarget
{
    ...
    uint64_t shadowEpoch; // observation-only epoch for 2-ahead/2-taken trace
    bool twoAheadFunctionalCarried;
    uint8_t twoAheadWindowCount;
    uint8_t twoAheadTakenCount;
    std::array<bool, 2> twoAheadWindowValid;
    std::array<Addr, 2> twoAheadWindowPC;
    std::array<Addr, 2> twoAheadWindowFallthrough;
    std::array<bool, 2> twoAheadTakenValid;
    std::array<Addr, 2> twoAheadTakenPC;
    std::array<Addr, 2> twoAheadTakenTarget;
    bool twoFetchMaterialized;
    uint64_t twoFetchBatchId;
    uint8_t twoFetchWindowSlot;
    uint8_t twoFetchBatchSize;
    FetchTargetId twoFetchParentFetchId;
    bool twoFetchConsumed;
    unsigned twoFetchConsumedInsts;
    Tick twoFetchConsumeTick;
    uint8_t twoFetchPredictionKind;
    uint32_t twoFetchRejectReason;
    bool twoFetchPredictorBacked;
    bool twoFetchPredictionAccepted;
    bool twoFetchPredHit;
    bool twoFetchPredTaken;
    bool twoFetchPredReturn;
    bool twoFetchPredIndirect;
    bool twoFetchParentInvalidated;
    uint32_t twoFetchUsefulnessIndex;
    uint64_t twoFetchStreamId;
    uint64_t twoFetchEpoch;
    Tick twoFetchMaterializeTick;
    uint8_t twoFetchPrefetchState;
    uint32_t twoFetchSupplyRisk;
    bool twoFetchCostBackpressureAccounted;
    bool twoFetchCostPromotionMissAccounted;
    bool twoFetchCostPrefetchUsefulAccounted;
   ...
   FetchTarget()
       : tid(0),
         ...
         s3Source(-1),
         shadowEpoch(0),
         twoAheadFunctionalCarried(false),
         twoAheadWindowCount(0),
         twoAheadTakenCount(0),
         twoFetchMaterialized(false),
         twoFetchBatchId(0),
         twoFetchWindowSlot(0),
         twoFetchBatchSize(1),
         twoFetchParentFetchId(0),
         twoFetchConsumed(false),
         twoFetchConsumedInsts(0),
         twoFetchConsumeTick(0),
         twoFetchPredictionKind(0),
         twoFetchRejectReason(0),
         twoFetchPredictorBacked(false),
         twoFetchPredictionAccepted(false),
         twoFetchPredHit(false),
         twoFetchPredTaken(false),
         twoFetchPredReturn(false),
         twoFetchPredIndirect(false),
         twoFetchParentInvalidated(false),
         twoFetchUsefulnessIndex(0),
         twoFetchStreamId(0),
         twoFetchEpoch(0),
         twoFetchMaterializeTick(0),
         twoFetchPrefetchState(TWO_FETCH_PREFETCH_NOT_ISSUED),
         twoFetchSupplyRisk(TWO_FETCH_SUPPLY_RISK_NONE),
         twoFetchCostBackpressureAccounted(false),
         twoFetchCostPromotionMissAccounted(false),
         twoFetchCostPrefetchUsefulAccounted(false)
   {
       predMetas.fill(nullptr);
       predBTBEntries.clear();
       updateBTBEntries.clear();
       twoAheadWindowValid.fill(false);
       twoAheadWindowPC.fill(0);
       twoAheadWindowFallthrough.fill(0);
       twoAheadTakenValid.fill(false);
       twoAheadTakenPC.fill(0);
       twoAheadTakenTarget.fill(0);
   }
