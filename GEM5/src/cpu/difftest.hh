...
class RefProxy
{
  public:
    ...
    virtual void regCopy(void *dut, bool direction)
    {
        assert(regcpy);
        regcpy(dut, direction);
    }
    virtual vaddr_t guidedExec(void *guide)
    {
        assert(guided_exec);
        return guided_exec(guide);
    }
...
class NemuProxy : public RefProxy
{
  public:
    ...
    void regCopy(void *dut, bool direction) override;
    vaddr_t guidedExec(void *guide) override;

  private:
    struct NemuRiscv64Regfile
    {
        union
        {
            uint64_t _64;
    ...
    static_assert(offsetof(NemuRiscv64Regfile, v) == sizeof(uint64_t) * 83,
                  "NEMU RV64 base regfile layout must match legacy DIFFTEST_REG_SIZE");
    static_assert(offsetof(NemuRiscv64Regfile, vr) == sizeof(uint64_t) * 100,
                  "NEMU RV64 H-extension regfile layout must match RVH DIFFTEST_REG_SIZE");
    static_assert(sizeof(NemuRiscv64Regfile) == sizeof(uint64_t) * (100 + 32 * VENUM64 + 7),
                  "NEMU RV64 H+V regfile layout must match RVH+RVV DIFFTEST_REG_SIZE");

    struct NemuExecutionGuide
    {
        bool force_raise_exception;
        uint64_t exception_num;
        uint64_t mtval;
    ...
    NemuRiscv64Regfile nemuRegFile;
    NemuExecutionGuide nemuGuide;
    void (*nemuGuidedExec)(void *guide) = nullptr;
